...
Cavebot_Trajectory_t Motion_Trapezoidal(const Bsp_Meter_t start, const Bsp_Meter_t distance, const Bsp_Radian_t angle, const Bsp_Second_t delta_time)
{
    ...
    // {
    //     trajectory.linear_velocity = Bsp_Clip((Cavebot_GetLinearVelocity() + (kMotion_LinearAcceleration * delta_time)), kMotion_MinimumLinearVelocity, kMotion_MaximumLinearVelocity);
    // }
    // else
    // {
    //     trajectory.linear_velocity = sqrt(2.0 * kMotion_LinearAcceleration * fmax(0, distance));
    // }
